...
export class AuthJwtService {
  ...
  public async generateToken(account: {id: number, username: string, rol: string}): Promise<Tokens> {
      ...
      const expiresIn: string = this.configService.get<string>('EXPIRES_IN');
  ...
  public async createTokenForInvitation(payload: { institutionId: number, jti: string }) {
  ...
  public async verifyInvitationToken(token: string): Promise<{ institutionId: number, jti: string}>{
    try {
