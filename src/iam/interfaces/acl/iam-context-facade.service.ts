...
export class IamContextFacadeService {
  ...
  public async obtainUsernameByAccountId(accountId: number): Promise<string> {
    ...
    public async obtainInvitationUrl(institutionId:number, jti: string): Promise<string> {
        const createTokenForInvitationCommand = new CreateTokenForInvitationCommand(institutionId, jti);
        return await this.accountCommandService.handleCreateTokenForInvitation(createTokenForInvitationCommand);
    }

    public async obtainInvitationTokenPayload(token: string): Promise<{ institutionId:number, jti: string}> {
