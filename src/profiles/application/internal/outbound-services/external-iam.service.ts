...
export class ExternalIamService {
    ...
    public async obtainInvitationUrl(instituteId: number, jti: string): Promise<string> {
        return await this.iamContextFacade.obtainInvitationUrl(instituteId, jti);
    }
    ...
    public async obtainInformationFromTokenInvitation(token: string): Promise<{ institutionId: number, jti: string }> {
